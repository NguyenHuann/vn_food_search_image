...
// 2. SỰ KIỆN NÚT TẢI ẢNH (PHẦN BẠN BỊ THIẾU ĐÃ ĐƯỢC BỔ SUNG LẠI)
if (btnUpload) {
...
if (elDrop) {
  ...
  // Bấm vào vùng drop cũng mở upload
  elDrop.addEventListener("click", (e) => {
    // Tránh xung đột nếu bấm vào input
...
async function startCamera() {
  try {
    ...
    alert(
      "Không thể truy cập Camera. Vui lòng kiểm tra quyền hoặc dùng nút 'Tải ảnh'.",
    );
    closeCamera();
  }
}
...
function handleFileSelect(file) {
  ...
  if (!validTypes.includes(file.type)) {
    setError(
      "Định dạng file không hợp lệ. Vui lòng chọn ảnh (JPG, PNG, WEBP...).",
    );
    return;
  }
...
if (elBtnSearch) {
  elBtnSearch.addEventListener("click", async () => {
    ...
    try {
      ...
      const sameList = (data.results.same_dish || []).map((i) => ({
        ...i,
        isCorrect: true,
      }));
      const relatedList = (data.results.related || []).map((i) => ({
        ...i,
        isCorrect: false,
      }));
...
function renderResultsPage() {
  ...
  if (allResults.length === 0) {
    elMainGrid.innerHTML =
      '<div style="grid-column:1/-1;text-align:center;color:#fff;padding:20px;">Không tìm thấy kết quả nào.</div>';
    return;
  }
...
function createImageTile(item) {
  ...
  tile.innerHTML = `
        <img src="${url}" loading="lazy" alt="Food" onerror="this.src='https://via.placeholder.com/300x200?text=Error'"/>
...
function renderMeta(meta) {
  ...
  box.innerHTML = `
        ...
        <ul>${ingredients.length ? ingredients.map((i) => `<li>${i}</li>`).join("") : "<li>Đang cập nhật...</li>"}</ul>
        <h3><i class="fas fa-fire"></i> Cách làm:</h3>
        <div class="meta-step-scroll"><ol>${steps.length ? steps.map((s) => `<li>${s}</li>`).join("") : "<li>Đang cập nhật...</li>"}</ol></div>
    `;
}
